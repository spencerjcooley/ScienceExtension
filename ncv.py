...
for i_outer_fold, (train_idx, test_idx) in enumerate(kf_outer.split(range(len(PATIENT_DATA)))):
    ...
    for i_parameter_config, parameter_config in enumerate(parameter_configs):
        print(f"    [CONFIG {i_parameter_config+1}/{PARAMETER_ITER}]")
        fold_losses = []
        ...
        print(f"    [CONFIG {i_parameter_config+1}/{PARAMETER_ITER}] | Avg Val Loss: {mean_loss:.4f}")
    ...
    outer_test_loss = evaluate_model(model=model, dataloader=outer_test_loader, device=DEVICE)
    print(f"[OUTER {i_outer_fold+1}/{OUTER_K}] | Outer Loss: {outer_test_loss:.4f}")